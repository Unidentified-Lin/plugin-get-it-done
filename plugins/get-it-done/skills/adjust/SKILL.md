...
`GID_BASE` = 該目標的 worktree（`<repo>.gid-goals/<slug>`）。本 skill 後續所有 `.get-it-done/...` 路徑都在 `"$GID_BASE/.get-it-done/..."` 之下，所有 `python3 "$GID" <cmd>`（除 `git-preflight`/`goals`/`goal-worktree-init`）都帶 `--base "$GID_BASE"`。

```
GID := "${CLAUDE_PLUGIN_ROOT}/skills/continue/scripts/gid.py"
若本視窗已建立 GID_BASE → 沿用（用 python3 "$GID" goals 驗證仍存在）。
否則 python3 "$GID" goals：0 → 若 repo root 有舊 .get-it-done/state.md 走單目標 back-compat（GID_BASE unset）；1 → 用它；≥2 → 詢問 user 要修訂哪個 slug。export GID_BASE。
```
