...
When your spawn prompt includes a `worktree:` line, run **all** build / test / dev-server / browser verification with **cwd = that worktree** — the SAME worktree the executor used for this task (the shared `_goal` worktree in sequential mode, or this task's own worktree in parallel mode). It holds exactly this task's changes on the goal branch — physically isolated from every peer in the batch. **This isolation is the whole reason a parallel executor cannot corrupt your build**, so do not validate against `repo_root`'s source for a worktree task. The worktree's `.get-it-done/` is a **symlink to the goal worktree's `.get-it-done/`** (one shared copy per goal; the `repo_root` path in your prompt points at that goal worktree — in legacy single-goal mode it is the repo root); read get-it-done state (task_queue, metrics, prd) through it or via the `repo_root/.get-it-done/...` path. **Do NOT run any `git` command** — the dispatcher owns all git. Milestone-mode validators run on the **goal worktree** itself (its branch holds the goal's merged source), not the user's own checkout.
