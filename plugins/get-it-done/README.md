...
**Git 隔離（git 專案，v1.4.0 multi-goal）**：每個目標在**自己的 worktree**（`<repo>.gid-goals/<slug>`，分支 `gid/goal-<slug>`，從使用者 HEAD 切出）中執行，該 worktree **內含自己的 `.get-it-done/`**（透過 worktree 的 `info/exclude` 對 git 隱藏）。dispatcher 在 repo root 執行、但用 **`GID_BASE`**（goal worktree 路徑，`gid.py` 帶 `--base`）指定目標。多個視窗可各設不同 `GID_BASE`，在同一個 repo 上**同時並行多個目標**（各自獨立 worktree + index，git-safe、不需跨 session 鎖）。**整個目標的原始碼異動都累積在 goal 分支** —— 使用者自己的分支與工作目錄保持乾淨。`GID_BASE` 未設時 = repo root 單目標 back-compat。`gid.py goals` 列出進行中的目標、`gid.py goal-reset` 清除單一目標而不影響其他。**預設平行、由計畫驅動**：互相獨立（相依已清、`Touches` 不重疊）的任務會**自動並行**，各自開 task worktree（從 goal 分支切出、完成後 squash-merge 回 goal 分支），上限為 `max_parallel`（預設 5）/ `max_worktrees`；有相依或改到同檔的任務則**自動序列化**；當只有單一任務可動工時直接在 `_goal` 內跑。executor 與其 validator 共用同一個 task worktree；通過後在 goal 分支上 commit 一筆。設 `max_parallel: 1` 可改為完全循序。每個 milestone 驗證通過收整為 goal 分支上**單一 commit**（不保留中間 commit）。所有 worktree 共用**同一份** symlink 的 `.get-it-done/`。目標完成後 `gid/goal-<slug>` 留給使用者自行 review/merge/PR（不自動合併）。所有 git 操作由 `gid.py` 執行，狀態記在 `.get-it-done/git_state.json`。非 git 專案 fallback 為直接編輯 + 排程防護。
...
| 類型 | 名稱 | 用途 |
|---|---|---|
| Command | `/blueprint <需求>` | 互動式規劃流程（Intake → Requirements Confirmation → Change/Impact Scope Inventory → Implementation Direction → Task Detailing → Plan Freeze & Handoff）— 需求確認、範疇分析（scope-scanner + scope-verifier）、逐點討論、任務拆解凍結，完成後初始化 `.get-it-done/` 讓 `/continue` 接手自主執行 |
| Command | `/objective <目標>` | 重置每次目標的相關狀態，清空 `.get-it-done/workspace/`、刪除舊 PRD 與舊 RQ-* findings，初始化 v2 schema，啟動第一個派發器循環 |
| Command | `/adjust <修訂訊息>` | 階段性開發中修訂目標 — **soft** 在 `goal.md` 附加澄清/約束、保留 task_queue/prd/findings/workspace；**hard** 重寫 Goal/Success 並重置 planner artifacts。兩者皆保留 `progress_log.md`、`validation_log.md`、`.get-it-done/context/`、A-side learnings。若 dispatcher 為 RUNNING 會自動先暫停為 AWAITING_HUMAN |
| Command | `/continue` | Dispatcher 自動駕駛 — 從目標一路跑到 COMPLETE，不會因 context 預算主動停。**停下時機由規劃時決定**：planner 在 milestone 上標 `PauseAfter: true`（自然人工驗收邊界，例如 UX 感受、實機測試、stakeholder 簽核）時，dispatcher 會在該 milestone 通過後寫 `[PLANNED_PAUSE]` 並 soft EXIT（phase 仍為 EXECUTING）。其餘停下條件：phase 變為 COMPLETE / AWAITING_HUMAN / IDLE，或 planner singleton 剛 crash 需要等候 |
| Command | `/review` | 對目前變更執行 universal engineering checklist 審查（正確性、錯誤處理、安全性、資料完整性、效能、風格） |
...
| **B — 單一專案狀態 + 學習** `.get-it-done/`（= `<project>/.get-it-done/`） | 該使用者當前專案目錄，跟著 git 走 | 單一專案，可與隊友共享 | 可寫 | 目標 / PRD / 任務 / 日誌 + 專案專屬知識（領域、技術棧、程式碼地圖、決策、利害關係人） |
...
位於 `<project>/.get-it-done/`，由 `templates/.get-it-done/` 初始化：
...
# 5. 檢視狀態
cat .get-it-done/state.md                                       # 目前 phase + 最後一個 handoff
cat .get-it-done/progress_log.md                                # 完整執行歷史
cat .get-it-done/validation_log.md                              # 驗證歷史
cat .get-it-done/context/domain_knowledge.md                    # 這個專案教給 Reflector 的事
...
注意：`.get-it-done/workspace/` 不在 plugin templates 內 —— 由 bootstrap 在專案 `.get-it-done/` 下建立，每個 executor 寫自己的 `.get-it-done/workspace/exec-<task_id>/` scratch dir。`/objective` 會清空整個 `.get-it-done/workspace/`。
