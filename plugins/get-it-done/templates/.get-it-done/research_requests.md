...
- `Claimed_by` is owned by the dispatcher — Planner never writes it. The dispatcher sets it just before spawning the analyst and clears it on persist. A non-null `Claimed_by` with `Status: open` is the crash-recovery signal (see `.get-it-done/STATE_SPEC.md` crash detection contract).
