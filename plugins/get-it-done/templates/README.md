...
## `templates/.get-it-done/` —— 單一專案（B 側）

| → 目的地 | `<project>/.get-it-done/` |
|---|---|
| 解析路徑 | 使用者當前工作目錄（呼叫 `/objective` 的位置） |
| 範圍 | **單一專案**，可透過 git 與隊友共享 |
| 撐過 plugin 更新嗎？ | ✅ 會（住在使用者的 repo 裡） |
| 進 git 嗎？ | ✅ 通常會（使用者也可以 `.gitignore` 掉） |
| 內容 | 每次目標的執行期狀態（state、goal、tasks、metrics、findings、日誌），以及 `.get-it-done/context/` —— 專案專屬知識（領域、技術棧、程式碼、決策、利害關係人）。PRD 不在 templates 內 —— Planner 視需要才寫 `.get-it-done/prd.md`，`/objective` 會在每個新目標前刪除舊的。 |
...
### macOS / Linux（Claude Code 和 GitHub Copilot）
...
# 另外建立 .get-it-done/workspace/（v2：每個 executor 在底下開自己的 exec-<task_id>/）
mkdir -p .get-it-done/workspace
```

### Windows（GitHub Copilot — PowerShell）

`rsync` 在 Windows 上預設不可用，改用 `robocopy`（Windows 內建）：
...
# 建立 workspace
New-Item -ItemType Directory -Path ".get-it-done\workspace" -Force | Out-Null
```

`robocopy` 旗標說明：`/E` = 包含子目錄，`/XC /XN /XO` = 跳過目的地已存在的檔案（等同 `--ignore-existing`），`/NFL /NDL /NJH /NJS` = 抑制輸出。

兩種方式都具備冪等性 —— 全部都存在時，下一次執行複製 0 個檔案。
