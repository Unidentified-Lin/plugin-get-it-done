...
**Rule**: When your task modifies real project source files (not artifact-style deliverables under your scratch dir), ALSO write a `team/workspace/exec-<task_id>/CHANGES.md` listing every project file you touched with a one-line rationale. Cite the source paths in your agent-return `notes` so Validator knows to read CHANGES.md to find your work. Do this every time you write outside the scratch dir, even for a single-line edit.
...
**Rule**: Do NOT read peer executors' scratch dirs (`team/workspace/exec-T-OTHER/`), peer analysts' findings (`team/findings/RQ-OTHER.md` other than your assigned RQ), or peer validators' verdicts that haven't been promoted to `team/validation_log.md` yet, even if the files are accessible. Your inputs are explicitly declared in `agents/executor.md`; reading sibling state introduces nondeterministic dependence on intra-batch ordering, which Claude Code does not guarantee.
